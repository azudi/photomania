 ...
 let food=[
  ...
  export default function homepagereducer(state=food,action){
      switch(action.type){
        default:
            return state;
            case actions.MSG_ADDED:
                return (state)
              case actions.MSG_REMOVED:
                if(state[0].length<=1){
                     return([food.filter(e=>e.id===Number(action.payload.id)),food.filter(e=>e.id!==Number(action.payload.id))])
                }
                 else{
                return([state.filter(e=>e.id===Number(action.payload.id)),food.filter(e=>e.id!==Number(action.payload.id))])
          }
        }
  }
